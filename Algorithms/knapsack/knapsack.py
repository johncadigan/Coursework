...
#         items = sorted(items, key=lambda a: float(a[0])/float(a[1]), reverse=True)
     ...
     def main(self):
         for i in xrange(1,len(self.values)+1):
             ...
             cur_weights = sorted(list(self.A[i].keys()))
             if i % len(self.values)/4 == 0:
                 print "{0} percent done".format(float(i)/float(len(self.values))) 
             for n, x in enumerate(cur_weights):
                 ...
                 if x-self.weights[i] >= 0:
                      if self.A[i-1].has_key(x-self.weights[i]):
                          ...
                          l = sorted(filter(lambda a: a < x-self.weights[i], self.A[i-1].keys()))[-1]
                          lower_w_v = self.A[i-1][l]
                      self.A[i][x] = max(self.A[i-1][x], lower_w_v+self.values[i])
                 else:
                      self.A[i][x] = self.A[i-1][x]
                 #Adding new values based on current item
                 if self.verbose: print self.A
                 new_weight = x+self.weights[i]
                 l = sorted([a for a in self.A[i-1].keys() if a < new_weight])
                 last_weight = l[-1]
                 new_value = self.values[i]+self.A[i-1][x]
                 if self.A[i].has_key(new_weight)== False and new_weight <= self.capacity and self.A[i][last_weight] < new_value:
                      if self.verbose:  print "Adding new w {0} v {1} based on last weight {2}".format(new_weight, new_value, last_weight)
                      self.A[i][new_weight] = new_value
                      if self.verbose: print self.A   
             if i+1 <= len(self.values): self.A[i+1] = dict(self.A[i])
