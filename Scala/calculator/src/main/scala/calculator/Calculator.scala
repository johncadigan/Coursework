...
object Calculator {
  ...
  def computeValues(
      ...
      if(Calculator.accumulateRefs(v(), namedExpressions, List(k))==true)  
        Signal(eval(namedExpressions(k)(), namedExpressions))
      else Signal(Double.NaN)
