...
trait GameDef {
  ...
  case class Block(b1: Pos, b2: Pos) {
    ...
    def left = if (isStanding)         dy(-2, -1)
               else if (b1.x == b2.x)  dy(-1, -2)// makes it stand
               else                    dy(-1, -1) // roll left

    /** The block obtained by moving right */
    def right = if (isStanding)        dy(1, 2)
                else if (b1.x == b2.x) dy(2, 1)//makes it stand
                else                   dy(1, 1)// roll right

    /** The block obtained by moving up */
    def up = if (isStanding)           dx(-2, -1)
             else if (b1.x == b2.x)    dx(-1, -1)// roll up
             else                      dx(-1, -2)// stand up

    /** The block obtained by moving down */
    def down = if (isStanding)         dx(1, 2)
               else if (b1.x == b2.x)  dx(1, 1)// roll down
               else                    dx(2, 1)//stand up
